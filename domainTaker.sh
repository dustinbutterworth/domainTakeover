...
if [[ "$(docker images -q ice3man/subfinder:latest 2> /dev/null)" == "" ]]
then
    echo 'Error: subfinder latest docker image is not installed.' >&2
    exit 1
else
    printf "#######################################################\n"
    ...
    docker run -v $HOME/.config/subfinder:/root/.config/subfinder -it ice3man/subfinder -silent -d ${TARGET} | tee ./${TARGET}/subfinder.out
